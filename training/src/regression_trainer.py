...
def load_class_names(num_classes: int, class_json: Optional[Path] = None) -> List[str]:
    ...
    """
    class_file = class_json or (
        PROJECT_ROOT / "training" / "datasets" / "annotate" / "classes.json"
    )
    names: List[str] = []
...
def auto_detect_image_folders(base_path: Optional[Path]) -> Tuple[List[List[str]], Optional[Path]]:
    ...
    if base_path is None:
        ...
        if not base_path.is_absolute():
            ...
            _register_candidate(PROJECT_ROOT / "training" / "datasets" / base_path)
            _register_candidate(PROJECT_ROOT / "datasets" / base_path)

        # Add variant names
        if base_path.name.lower() in {"regression_dataset", "regressiondataset"}:
            _register_candidate(PROJECT_ROOT / "training" / "datasets" / "Regression_dataset")
